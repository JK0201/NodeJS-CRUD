...
module.exports = function (sessionChk, db, upload, fs, path) {
  ...
  router.post('/add-item', upload.array('upload', 3), async function (req, res) {
  ...
  // Send file
  router.get('/loadimage', async function (req, res) {
    req.query._id = parseInt(req.query._id);
    try {
  ...
  //Update post
  router.put('/update-item', upload.array('upload', 3), async function (req, res) {
